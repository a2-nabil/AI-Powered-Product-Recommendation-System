jQuery(document).ready(function ($) {
  ...
  function sendMessage() {
    ...
    const $thinking = $(
      '<div class="ai-recommender-message ai-message"><div class="ai-recommender-thinking">Thinking...</div></div>'
    );
  ...
  function processImageSearch(imageData) {
    ...
    const $message = $(
      '<div class="ai-recommender-message user-message"></div>'
    );
    $message.append('<div class="ai-recommender-avatar">You</div>');
    $message.append(
      '<div class="ai-recommender-image-upload"><img src="' +
        imageData +
    ...
    // Add thinking indicator
    const $thinking = $(
      '<div class="ai-recommender-message ai-message"><div class="ai-recommender-thinking">Analyzing image...</div></div>'
    );
    $chatbotMessages.append($thinking);
    $chatbotMessages.scrollTop($chatbotMessages[0].scrollHeight);
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (
          ...
          addMessage(
            "ai",
            "I couldn't find any matching products based on your image. Could you try a different image?"
          );
        }
      },
      error: function () {
  ...
  function displayProducts(products) {
    ...
    products.forEach((product) => {
      ...
      $product.append(
        '<a href="' +
          product.url +
          '" class="ai-recommender-product-link" target="_blank">View Product</a>'
      );
  ...
  if (aiRecommender.welcome_message) {
    ...
    addMessage(
      "ai",
      "Hello! I'm your AI product assistant. I can help you find products or answer questions about our store. You can also upload an image to find similar products."
    );
  }
});
